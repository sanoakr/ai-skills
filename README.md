...
# インストール済みスキルを更新 + 対応待ちリポジトリの説明 TSV を再生成（コミット・プッシュまで自動）
...
> `skills.conf` に `?` プレフィックスで管理。スキル数が多いため `setup-global.fish` の自動インストール対象外。  
> `gh skill install {リポジトリ} {スキル名}` で必要なスキルを個別にインストール可能。  
> インストール状況は `./setup-global.fish list` で確認できる。
...
| リポジトリ | 説明 |
|-----------|------|
| [K-Dense-AI/scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills) | 科学研究向けスキル集（論文執筆・文献検索・バイオインフォ・創薬・量子計算など 135 スキル） |
...
├── pending-descs/       # 対応待ちリポジトリのサブスキル日本語説明 TSV（update で生成）
