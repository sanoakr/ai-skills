...
# 全スキルをインストール（インストール先エージェントの明示が必須）
./setup.fish install claude-code
./setup.fish install claude-code codex     # 複数指定
./setup.fish install all                   # 全エージェント（claude-code codex github-copilot）
...
# エージェント省略時は claude-code のみにインストール
./setup.fish add <skill-name>@<owner/repo> [agent ...]

# 外部スキルを削除（コミット → プッシュまで自動）
./setup.fish remove <skill-name>

# リモートから pull（エージェントを指定するとインストールまで実行）
./setup.fish sync                # pull のみ
./setup.fish sync claude-code    # pull ＋ claude-code へインストール
```
