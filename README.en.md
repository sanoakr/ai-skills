...
# Fetch pending skill descriptions (commit & push)
...
```
ai-skills/
├── setup.fish           # Setup script (fish)
├── setup.sh             # Setup script (bash)
├── fetch_skill_descs.py # Fetch SKILL.md descriptions from pending repos
├── print_pending_list.py# Format and print pending skill list
├── skills.conf          # External skills list (edit to share)
├── skills.desc          # Japanese descriptions
├── pending-descs/       # Japanese description TSVs for pending repos (generated by fetch-pending)
